...
- **Added a field to `RT` and the app died before it started.** The V7 shadow build flashed on 2026-09-02 crashed instantly with no stack trace and no log line, because `RT` gained one property. `RT` is constructed inside `determine_basal` in every engine here, so one field costs one register in each; measured against the commit the phone was running, the crashing build carried exactly +1 register in all seven, and `DetermineBasalBoostV3MLG3.determine_basal` went 273 to 274, which it has no room for. `RT.kt` had carried a comment warning of this since 2026-07-18, three lines above where the field was inserted, and the comment was read after the crash rather than before the edit. → Telemetry rides in the `reason` string as a `tag=value; ` (as the KAIROS twin, tranche and shadow hypo score do) and is parsed in the extractor; `RtFieldCountTest` now pins the field count so the next attempt fails a test instead of a phone. When a file carries a comment about the change you are making, read it first.
